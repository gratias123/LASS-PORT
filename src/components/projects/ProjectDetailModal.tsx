...
import { NeutralProjectPlaceholder } from './NeutralProjectPlaceholder';
...
export const ProjectDetailModal: React.FC<ProjectDetailModalProps> = ({ project, onClose }) => {
  ...
  const allImages = React.useMemo(() => {
    ...
    const mainImg =
      ...
      (project.id && fallbackMap[project.id]) ||
      '';
    if (mainImg) {
      list.push(mainImg);
    }
          ...
          <div className="space-y-3">
            <div className="aspect-[16/9] w-full rounded-2xl overflow-hidden bg-slate-100 border border-slate-200/80 relative shadow-xs">
              {currentActiveImage ? (
                ...
                <NeutralProjectPlaceholder project={project} isEn={isEn} size="modal" />
              )}
