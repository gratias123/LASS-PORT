...
export const StepCertifications: React.FC<StepProps> = ({ data, onChange }) => {
  ...
  const handleUpdateCert = (index: number, field: keyof CustomCertificationItem, value: any) => {
      ...
      <div className="border-b border-slate-200 pb-3 flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold text-slate-900 font-heading flex items-center gap-2">
            ...
            <span>Section 7 — Certifications & Attestations</span>
          </h3>
          <p className="text-xs text-slate-500 mt-1">
            Certifications professionnelles obtenues, attestations en ligne et documents PDF vérifiables.
          </p>
        </div>
        <button
          type="button"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full text-xs px-2.5 py-1.5 bg-white border border-slate-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 font-mono"
                />
              </div>

              {/* PDF Document Attachment Sub-section */}
              <div className="sm:col-span-2 p-3 bg-white border border-slate-200 rounded-lg space-y-2">
                <div className="flex items-center justify-between">
                  <label className="block text-[11px] font-bold text-slate-700">
                    Document PDF du certificat officiel
                  </label>
                  {cert.documentUrl ? (
                    ...
                    <span className="text-[10px] text-slate-400">Aucun PDF lié</span>
                  )}
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-2 items-center">
                  <input
                    type="text"
                    placeholder="URL ou chemin du PDF (ex : /certificates/IA_responsable_Certificate(1).pdf)"
                    value={cert.documentUrl || ''}
                    onChange={(e) => handleUpdateCert(idx, 'documentUrl', e.target.value)}
                    className="flex-1 w-full text-xs px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg font-mono"
                  />
                  <label className="inline-flex items-center gap-1 px-3 py-1.5 bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-lg text-xs font-semibold cursor-pointer border border-blue-200 shrink-0">
                ...
                <div className="flex items-center gap-4 text-xs pt-1">
                  <label className="inline-flex items-center gap-1.5 text-slate-600 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => handleUpdateCert(idx, 'showVerifyButton', e.target.checked)}
                      className="rounded text-blue-600"
                    />
                    <span>Activer le bouton « Vérifier le certificat »</span>
                  </label>
                </div>
              </div>
            </div>
