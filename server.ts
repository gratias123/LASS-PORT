...
// Ensure public uploads and certificates directories exist and are statically served
...
// Serve /projects statically
app.use('/projects', express.static(projectsDir));

// Serve /certificates with proper inline headers for PDF viewing
app.use('/certificates', (req, res, next) => {
  ...
  next();
}, express.static(certsDir));

// Serve /uploads with proper inline headers
...
// Upload image or document endpoint (saves base64 data to public/uploads/ and returns URL)
app.post('/api/upload', (req, res) => {
  try {
    ...
    if (!rawData.startsWith('data:')) {
      res.json({ success: true, url: rawData, filename: filename || '' });
      return;
    }
    ...
    const cleanBaseName = (filename || (ext === 'pdf' ? 'certificat' : 'image'))
      .replace(/\.[a-zA-Z0-9]+$/, '')
      .toLowerCase()
      .replace(/[^a-z0-9]/g, '_')
      .replace(/_+/g, '_')
...
// Dedicated document upload (PDF / Attestations)
app.post('/api/upload-document', (req, res) => {
  try {
    const { document, filename } = req.body;
    if (!document || typeof document !== 'string') {
      res.status(400).json({ success: false, error: 'Document requis' });
      return;
    }

    if (!document.startsWith('data:')) {
      res.json({ success: true, url: document, filename: filename || '' });
      return;
    }

    const matches = document.match(/^data:([A-Za-z0-9-+\/.]+);base64,(.+)$/);
    if (!matches || matches.length !== 3) {
      res.status(400).json({ success: false, error: 'Format de fichier non reconnu' });
      return;
    }
    ...
    const originalName = filename ? path.basename(filename) : `certificat_${Date.now()}.${ext}`;
    const safeName = originalName
      .replace(/[^a-zA-Z0-9._-]/g, '_')
      .replace(/_+/g, '_');

    const finalFileName = safeName.toLowerCase().endsWith(`.${ext}`) ? safeName : `${safeName}.${ext}`;
    // Save in public/certificates or public/uploads
    ...
    res.status(500).json({ success: false, error: err.message || 'Erreur d’enregistrement du document' });
  }
});

// Check if a document file exists on the server to prevent broken links
app.get('/api/document/verify-exists', (req, res) => {
  ...
  if (docUrl.startsWith('http://') || docUrl.startsWith('https://')) {
    res.json({ exists: true, isExternal: true });
    return;
  }

  // Check local filesystem in public/
  const cleanPath = docUrl.replace(/^\//, '').split('?')[0];
  const fullPath = path.join(process.cwd(), 'public', cleanPath);
  const exists = fs.existsSync(fullPath);
  res.json({ exists, path: cleanPath });
});
