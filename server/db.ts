...
const DEFAULT_OWNER_PORTFOLIO_DATA = {
  ...
  skills: [
    ...
    { id: 'sk-23', name: 'Formulation et optimisation de prompts', category: 'Prompt Engineering', levelOrDesc: 'Rédaction de requêtes précises, calibrage de rôles, contraintes et critères de qualité.' },
    { id: 'sk-24', name: 'Structuration de contextes et d’instructions complexes', category: 'Prompt Engineering', levelOrDesc: 'Fourniture de contextes riches, variables dynamiques et décomposition d’instructions.' },
    { id: 'sk-25', name: 'Génération et adaptation de contenus textuels et visuels', category: 'Prompt Engineering', levelOrDesc: 'Production de contenus techniques, scénarios d’usage et déclinaisons visuelles.' },
    { id: 'sk-26', name: 'Automatisation de tâches répétitives assistée par IA', category: 'Prompt Engineering', levelOrDesc: 'Standardisation de processus, extraction et synthèse de données complexes.' },
    { id: 'sk-27', name: 'Prototypage rapide de solutions numériques assisté par IA', category: 'Prompt Engineering', levelOrDesc: 'Accélération du maquettage, simulation de scénarios et validation de concepts.' },
    { id: 'sk-28', name: 'Utilisation raisonnée et responsable des outils d’IA', category: 'Prompt Engineering', levelOrDesc: 'Évaluation critique, conformité éthique, respect de la confidentialité et des sources.' },
  ],
  experiences: [
    {
      id: 'exp-1',
    ...
    },
    {
      ...
      organization: 'Pratique personnelle & projets appliqués',
      location: 'Porto-Novo, Bénin',
      period: '2024 - Présent',
      current: true,
      description: 'Conception de requêtes structurées, automatisation de flux de travail et prototypage assisté par les modèles d’intelligence artificielle générative.',
      missions: [
  ...
  certifications: [
    {
      ...
      description: 'Maîtrise des pratiques quotidiennes sur le web : navigation sécurisée, outils en ligne courants et gestion des informations personnelles.',
    },
    {
      id: 'cert-2',
      ...
      showVerifyButton: true,
      description: 'Sensibilisation aux principes éthiques de l’IA : équité des algorithmes, transparence, protection de la vie privée et utilisation raisonnée.',
    },
    {
      id: 'cert-3',
      ...
      showVerifyButton: true,
      description: 'Compréhension des notions fondamentales de l’intelligence artificielle, de ses mécanismes généraux et de ses applications concrètes.',
    },
    {
      id: 'cert-4',
      ...
      showVerifyButton: true,
      description: 'Méthodes de recherche documentaire efficace, validation des sources en ligne et analyse critique des contenus trouvés sur le web.',
    },
    {
      id: 'cert-5',
      ...
      showVerifyButton: true,
      description: 'Connaissance des principes de fonctionnement d’Internet, de l’acheminement des données et des architectures réseau de base.',
    },
  ],
  projects: {
    enabled: true,
  ...
  tools: [
    ...
    { id: 'tool-15', name: 'ChatGPT & Gemini', category: 'Intelligence Artificielle', description: 'Ingénierie de requêtes avancée, assistance à la conception et synthèse textuelle.', icon: 'Sparkles', level: 'Maîtrisé' },
    { id: 'tool-16', name: 'Claude', category: 'Intelligence Artificielle', description: 'Raisonnement contextuel étendu, rédaction technique et structuration de données.', icon: 'Sparkles', level: 'Maîtrisé' },
  ],
  wikimedia: {
    enabled: true,
    username: 'Semako64',
  ...
  private ensureAdminAccount() {
    ...
    if (!adminPortfolio) {
      ...
      if (!currData.projects || !Array.isArray(currData.projects.items) || currData.projects.items.length === 0) {
        ...
        // Ensure default projects have their dedicated image URLs if missing
        currData.projects.items.forEach((p: any) => {
          if (!p.imageUrl) {
            const match = refData.projects.items.find((rp: any) => rp.id === p.id);
            if (match?.imageUrl) {
      ...
      if (currData.identity) {
        currData.identity.mainTitle = refData.identity.mainTitle;
      }
      if (currData.about) {
        currData.about.tagline = refData.about.tagline;
        currData.about.heroSummary = refData.about.heroSummary;
        currData.about.presentation = refData.about.presentation;
        currData.about.highlights = refData.about.highlights;
      }
      // Ensure Prompt Engineering skills exist
      if (Array.isArray(currData.skills)) {
        refData.skills.forEach((sk: any) => {
          if (!currData.skills.some((s: any) => s.id === sk.id || s.name === sk.name)) {
            currData.skills.push(sk);
          }
        });
      }
      // Ensure Prompt Engineering experience exists
      if (Array.isArray(currData.experiences)) {
        refData.experiences.forEach((exp: any) => {
          if (!currData.experiences.some((e: any) => e.id === exp.id || e.title === exp.title)) {
            currData.experiences.push(exp);
          }
        });
      }
      // Ensure proj-4 exists in projects and all projects have photorealistic images
      if (currData.projects?.items) {
        refData.projects.items.forEach((proj: any) => {
          const existing = currData.projects.items.find((p: any) => p.id === proj.id);
          if (!existing) {
            currData.projects.items.push(proj);
          } else if (proj.imageUrl && (!existing.imageUrl || !existing.imageUrl.trim() || existing.imageUrl !== proj.imageUrl)) {
            existing.imageUrl = proj.imageUrl;
          }
        });
      }
      // Ensure AI tools exist in tools
      if (Array.isArray(currData.tools)) {
        refData.tools.forEach((t: any) => {
          if (!currData.tools.some((tool: any) => tool.id === t.id || tool.name === t.name)) {
            currData.tools.push(t);
          }
        });
      }
